...
void makeTH3(RooWorkspace *bgws,const int popInd, const int time_bin)
{
  ...
  //const int comptonBottomComA[nc[1]]={1,2,3,4};
  int* comptonBottomComA = new int[nc[1]];
  comptonBottomComA[0] = 1;
  comptonBottomComA[1] = 2;
  comptonBottomComA[2] = 3;
  comptonBottomComA[3] = 4;
  TArrayI comptonBottomCom(nc[1],comptonBottomComA);
  componentIds[1]=comptonBottomCom;

  labels[2]="ComptonRest";
  //const int comptonRestComA[nc[2]]={0,5};
  int* comptonRestComA = new int[nc[2]];
  comptonRestComA[0] = 0;
  comptonRestComA[1] = 5;
...
//	  cout<<"rawS2, EventEfficiency(rawS2)  "<< rawS2 <<" ,"<<EventEfficiency(rawS2) <<"\n"<<endl;
       }
...
std::cout<<nSpaceBinsX<<nSpaceBinsY<<nSpaceBinsZ<<std::endl;
...
//		std::cout<<rbin<<phibin<<dtbin<<std::endl;
//		for (int ii=0; ii<10; ii++) std::cout<<"look at here!!!"<<std::endl;
   ...
   bgws->factory(TString::Format( "%s%d_expectation[%f]",labels[popInd].Data(),time_bin,counts )); //expected number of counts
   bgws->var(TString::Format("%s%d_expectation",labels[popInd].Data(),time_bin))->Print();
...
void spatialTH3FromNtuples_new()
{
