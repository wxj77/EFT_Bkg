...
void filter_FCS_Tl208_planB(){
    
    // Create chain with the original RQ files, containing all the events
    TChain *ch1 = new TChain("events");
    
    //cryostat w/o Th232E
    ch1->Add("/cms26r0/sjh/lux_bkg_model/planB_bkg_model/root_output/FCSHIELDPLANE_Tl208_summary.root");
    
    
    //    TChain *ch1  = (TChain*)gROOT->FindObject("events");
    
    // Open the root file where golden events will be written
    TFile *hfile = new TFile("/cms26r0/sjh/lux_bkg_model/planB_bkg_model/planB_BkgModelInputs/goldenFCS_Tl208.root","RECREATE");
