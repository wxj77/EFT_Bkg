...
void filter_Xe_Rn220_planB(){
    
    // Create chain with the original RQ files, containing all the events
    TChain *ch1 = new TChain("events");
    
    
    ch1->Add("/cms26r0/sjh/lux_bkg_model/planB_bkg_model/root_output/XENON_Rn220_summary.root");
    
    
    //    TChain *ch1  = (TChain*)gROOT->FindObject("events");
    
    // Open the root file where golden events will be written
    TFile *hfile = new TFile("/cms26r0/sjh/lux_bkg_model/planB_bkg_model/planB_BkgModelInputs/goldenXe_Rn220.root","RECREATE");
