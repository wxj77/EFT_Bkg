...
/*
const int nc[nPop];
nc[0]=3; //number inputs for each pdf
nc[1]=4;
nc[2]=2;
...
//  TFile *outfile = new TFile(TString::Format("Bkg_S1Log10S2_TH2F_%s_TB%d.root",labels[popInd].Data(),time_bin),"RECREATE");
  TFile *outfile = new TFile(TString::Format("./PDFs/Bkg_S1Log10S2_TH2F_%s_TB%d.root",labels[popInd].Data(),time_bin),"RECREATE");
  cout << outfile<<endl;
...
void responseTH2FromNtuples_new()
{
