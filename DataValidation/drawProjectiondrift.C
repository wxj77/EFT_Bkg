...
int drawProjection(TString datafile= "/home/wxj/EFT_Bkg/DataValidation/eft_bg_data_trial_05182018.root",int timeBin=1, int driftBin=1, TString histname="drift"){
...
if (histname=="r") {histTitle = "; r [cm]; event rate [cts kg^{-1} day^{-1} cm^{-1}]"; hxmin=0; hxmax=FIDRADIUS; hnbins= NRBIN;}
if (histname=="phi") {histTitle = "; phi ; event rate [cts kg^{-1} day^{-1}]"; hxmin=PHIMIN; hxmax=PHIMAX; hnbins= NPHIBIN;}
if (histname=="drift") {histTitle = "; drift time [us]; event rate [cts kg^{-1} day^{-1} us^{-1}]";  hxmin=DTMIN; hxmax=DTMAX; hnbins= NDTBIN;} 
if (histname=="s1") {histTitle = "; s1c [phd]; event rate [cts kg^{-1} day^{-1} phd^{-1}]";  hxmin=S1MIN; hxmax=S1MAX; hnbins= NS1BIN;}
...
TH1F* rk_drift= (TH1F*) rk.bkg_tb[timeBin-1].bkg_dt[driftBin-1].rphidrift->Project3D("z");
...
TH1F* cr_drift= (TH1F*) cr.bkg_tb[timeBin-1].bkg_dt[driftBin-1].rphidrift->Project3D("z");
cr_drift->SetDirectory(0);
cr_drift->Scale(tmass[timeBin-1][4]/tmass[timeBin-1][driftBin-1]);
TH1F* cb_drift= (TH1F*) cb.bkg_tb[timeBin-1].bkg_dt[driftBin-1].rphidrift->Project3D("z");
...
for (int ii=1; ii<= nn; ii++) {
  ...
  cout<<"test"<<rk_drift->GetMinimum()<<endl;
cout<<"test"<<rk_drift->GetMaximum()<<endl;  
...
cb_drift->SetLineColor(kMagenta+2);
